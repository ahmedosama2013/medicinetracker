...
This directory holds the backend: [`migrations/0001_init.sql`](migrations/0001_init.sql) (tables, RLS, the code-gated function surface, the freeze and reminder jobs), later migrations layered on top of it, and three Edge Functions — [`send-reminders`](functions/send-reminders), [`nudge`](functions/nudge) and [`supporter-photo`](functions/supporter-photo).
